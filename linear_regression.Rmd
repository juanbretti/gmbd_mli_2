...
\
The *adjusted R squared* is `r round(with(summary(model_step), 1 - deviance/null.deviance), 3)`.
...
\
Another important linear correlation is between **Bedrooms** and **Sq.Mt**.\
\
There is no visible correlation between the *Explanatory/Exogenous* variables.
...
The values are the *log10* of the response.


```{r}
temp_lag <- tibble(t0 = predict(model_step, data4) - data4$Rent,
                       `t-1` = lag(predict(model_step, data4) - data4$Rent, 1))
plot(log10(temp_lag), main = 'Correlation between log residual observations')
```
